...
def meansToCSV(name_alg, name_db, mean_results):
    alg_to_index = {'KNN':1,'SFS':2, 'AM (10, 1)':3,'AM (10, 0.1)':4,'AM (10, 0.1-mej)':5}
...
# Función para obtener un número sorteado siguiendo una distribución
def roulette(vector_prob):

    random = np.random.random()
    accumulated = 0
    index = 0

    while random >= accumulated:
        accumulated += vector_prob[index]
