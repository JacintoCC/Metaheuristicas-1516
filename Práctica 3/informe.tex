...
\lstset{language=C,
		otherkeywords={FOR,BEGIN,WHILE,IF,END,THEN,ELSE,DO,RETURN},
        basicstyle=\ttfamily,
...
BEGIN
   ...
   chromosomes $\leftarrow$ {{random{T,F}: size = num_features }:
                                               size = 30}
   scores $\leftarrow$ {score(data[chrom],categories): 
                                     chrom $\in$ chromosomes} 
   
   population $\leftarrow$ concatenate( (chromosomes, scores), 
                                              by columns)
...
BEGIN
   ...
   FOR i IN {0,2,4,...,length(parents)-2} BEGIN
      ...
      a,b $\leftarrow$ {random{1,$\dots$,num_features-1}, size = 2,
                                               replace=False}
...
BEGIN
   ...
   chromosomes $\leftarrow$ {random(descendants_chromosomes), 
                                    size = num_gens_to_mutate}
   genes $\leftarrow$ {random({0,...,num_features-1}), 
                                    size = num_gens_to_mutate}
