...
\title{	Práctica 5 Metaheurísticas.\\
...
% Tablas AM1
\pgfplotstableread[col sep=comma]{Software/Resultados/wAM1.csv}\datawAMa
\pgfplotstableread[col sep=comma]{Software/Resultados/lAM1.csv}\datalAMa
\pgfplotstableread[col sep=comma]{Software/Resultados/aAM1.csv}\dataaAMa

% Tablas AM01
\pgfplotstableread[col sep=comma]{Software/Resultados/wAM01.csv}\datawAMb
\pgfplotstableread[col sep=comma]{Software/Resultados/lAM01.csv}\datalAMb
\pgfplotstableread[col sep=comma]{Software/Resultados/aAM01.csv}\dataaAMb

% Tablas AM01m
\pgfplotstableread[col sep=comma]{Software/Resultados/wAM01m.csv}\datawAMc
\pgfplotstableread[col sep=comma]{Software/Resultados/lAM01m.csv}\datalAMc
\pgfplotstableread[col sep=comma]{Software/Resultados/aAM01m.csv}\dataaAMc

% Tablas AM1AI
\pgfplotstableread[col sep=comma]{Software/Resultados/wAM1AI.csv}\datawAMaAI
\pgfplotstableread[col sep=comma]{Software/Resultados/lAM1AI.csv}\datalAMaAI
\pgfplotstableread[col sep=comma]{Software/Resultados/aAM1AI.csv}\dataaAMaAI

% Tablas AM01AI
\pgfplotstableread[col sep=comma]{Software/Resultados/wAM01AI.csv}\datawAMbAI
\pgfplotstableread[col sep=comma]{Software/Resultados/lAM01AI.csv}\datalAMbAI
\pgfplotstableread[col sep=comma]{Software/Resultados/aAM01AI.csv}\dataaAMbAI

% Tablas AM01mAI
\pgfplotstableread[col sep=comma]{Software/Resultados/wAM01mAI.csv}\datawAMcAI
\pgfplotstableread[col sep=comma]{Software/Resultados/lAM01mAI.csv}\datalAMcAI
\pgfplotstableread[col sep=comma]{Software/Resultados/aAM01mAI.csv}\dataaAMcAI

% Tabla Resumen
\pgfplotstableread[col sep=comma]{Software/Resultados/results.csv}\dataGlobal

% Tabla Resumen
\pgfplotstableread[col sep=comma]{Software/Resultados/resultsAI.csv}\dataGlobalAI
...
\subsubsection{Algoritmo memético con optimización sobre todos los individuos}
% Tablas Resumen AM1
\makeresume{\dataAMa}{\datawAMa}{\datalAMa}{\dataaAMa}

% Salida por pantalla
\maketable{\dataAMa}

Como pasa con los demás algoritmos implementados hasta ahora, se observa el incremento esperable con respecto al \texttt{KNN} y con respecto a \texttt{SFS} (salvo en \texttt{Arrhythmia} donde el algoritmo \textit{greedy} sigue siendo más efectivo). 


\subsubsection{Algoritmo memético con probabilidad de optimización 0.1}
% Tablas Resumen AM01
\makeresume{\dataAMb}{\datawAMb}{\datalAMb}{\dataaAMb}

% Salida por pantalla
\maketable{\dataAMb}

Con la optimización local sólo en un $10\%$ de los individuos se observa que los resultados son muy similares a optimizar toda la población. Sin embargo las diferencias en el tiempo de ejecución en la base de datos \texttt{Arrhythmia} sí que son significativas.


\subsubsection{Algoritmo memético con optimización sobre los mejores individuos}
% Tablas Resumen AM01m
\makeresume{\dataAMc}{\datawAMc}{\datalAMc}{\dataaAMc}

% Salida por pantalla
\maketable{\dataAMc}

Con la optimización sobre los mejores individuos se obtienen resultados muy similares a los de los otros algoritmos meméticos, observando que en tiempos de ejecución se vuelve a los tiempos de ejecución en \texttt{Arrhythmia} de la aplicación de la BL a todos los individuos.
...
\begin{axis}[
    ...
    ylabel=Tiempo (s),
    ymin = 0,
    legend style={at={(1.5,0.7)},anchor=north},
...
\legend{AM - 1, AM - 0.1, AM - 0.1 mejor}
\end{axis}
\end{tikzpicture}
\end{center}
...
\begin{axis}[
    ...
    ymin = 50,
    legend style={at={(0.5,-0.15)},anchor=north,legend columns=-1},
    xticklabels from table={\dataGlobalAI}{algorithm}
    %,colorbrewer cycle list=Set1
]
\addplot table [
    x=0,
...
\pgfplotstabletranspose[input colnames to=DB,columns={TW,TL,TA}]\dataTimesAI{\dataGlobalAI}

\begin{adjustwidth}{-1cm}{}
\begin{center}
\pgfplotstabletypeset[string type]\dataTimesAI
\end{center}
\end{adjustwidth}
...
\addplot[sharp plot, color=green]
	table [x=DB, y=0]	{\dataTimesAI};
\addplot[sharp plot, color=green, dashed]
	table [x=DB, y=1]	{\dataTimesAI};
\addplot[sharp plot, color=blue]
	table [x=DB, y=2]	{\dataTimesAI};
\addplot[sharp plot, color = blue, dashed]
	table [x=DB, y=3]	{\dataTimesAI};
\addplot[sharp plot, color = red]
	table [x=DB, y=4]	{\dataTimesAI};
\addplot[sharp plot, color = red, dashed]
	table [x=DB, y=5]	{\dataTimesAI};
\addplot[sharp plot]
	table [x=DB, y=6]	{\dataTimesAI};
\addplot[sharp plot, dashed]
	table [x=DB, y=7]	{\dataTimesAI};
\legend{AGG-H, AGE-H, 
		AM - 1, AM - 1- AI, 
		AM - 0.1, AM - 0.1- AI, 
		AM - 0.1 mejor, AM - 0.1 mejor - AI}
\end{axis}
\end{tikzpicture}
\end{center}
